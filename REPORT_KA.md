...
ეს დოკუმენტი არის პროექტის **ტექნიკური რეპორტი**. იგი ასაბუთებს ყველა მთავარ
...
დამოკიდებული წინასწარ გაწვრთნილ რესურსებზე. სწორედ ეს იძლევა სუფთა, რაოდენობრივი
პასუხის გაცემის შესაძლებლობას შეტევის წარმატებასა და დაცვის მდგრადობაზე.
...
ის კაუზალური ნიღბის ქვეშ ყველა წინა ტოკენიდან წინასწარმეტყველებს მომდევნო ტოკენს.
...
**რატომ ფლობს შემტევი CFG-ს.** შემტევმა რესურსები არ უნდა დახარჯოს გრამატიკის ხელახლა
...
`ignore_index=pad_id` უზრუნველყოფს, რომ padding ტოკენები გრადიენტში წვლილს არ შეიტანენ.
cross-entropy არის სტანდარტული, კარგად დამკვიდრებული MLE მიზანი ავტორეგრესიული მოდელებისთვის —
...
ერთი სტრუქტურირებული reward აფასებს ყოველ ურთიერთქმედებას. ის **სამ დონეზე ანიჭებს პრიორიტეტს**: გრამატიკული მარცხი ≫ თემატური შეუსაბამობა ≫ თემატური გადახრა.
...
იჭერს ამ თანდათანობით გადახრას, რომელსაც მხოლოდ Jaccard ვერ ხედავს. ამ reward-ის სტრუქტურის წყალობით შემტევს აქვს გლუვი გრადიენტი ასასვლელად იმ ეპიზოდებშიც, სადაც ჯერ
...
მაგალითად, თუ `FALL BURN` უკვე გამოჩნდა, `BURN`-ის `FALL`-ის შემდეგ გამეორება
...
Cross-entropy-ის წევრი მუდმივად „ამაგრებს" მოდელის გამავალ განაწილებას წინასწარ გაწვრთნილ
...
MiniGPT გაწვრთნილია 10,000-წინადადებიან კორპუსზე 32 ეპოქის განმავლობაში (AdamW, lr 3e-4,
...
- მკაფიო გადაკვეთა ~700-ე ეპიზოდთან, სადაც შემტევი აღმოაჩენს პრეფიქს-ფორმებს, რომლებიც
...
- ჩაშენებული გაკვეთილები: შემტევის ფაზები იყენებენ **ენტროპიის ბონუსს** (mode-collapse-ის
  ...
  state tracker უზრუნველყოფს, რომ ყოველი პრეფიქსი შევსებადია, ასე რომ ვერცერთი მხარე
...
არის ჩვენი ძირითადი, ცალსახა მეტრიკა, ხოლო *საშუალო reward* — მეორეული მეტრიკა, რადგან
...
*კითხვა:* მართლა „ლაპარაკობს“ თუ არა წინასწარ გაწვრთნილი დამცველი ენაზე?
...
### ამოცანა 2 — შეტევის წარმატება (REINFORCEMENT LEARNING-მდე vs. შემდეგ)
...
*კითხვა:* შეიძლება თუ არა დამცველი გავამაგროთ გაწვრთნილი შემტევის წინააღმდეგ ისე, რომ
...
საბაზისო (მხოლოდ cross-entropy-ით გაწვრთნილ) მოდელთან შედარებით?
...
| 4 | **გრამატიკის ცვლილება** — დაემატა `VERB_TERM → VERB VERB_TERM` (ზმნათა ჯაჭვი, მაგ. `MAN RUN FALL`). | ახალმა წესმა გააუქმა კორპუსი, დაგენერირებული დამცველი და state machine.                                  | ხელახლა დაგენერირდა კორპუსი, **ხელახლა გაწვრთნილი** დამცველი და განახლდა `CFGStateTracker`. |
...
*განხორციელების დეტალები (კოდის სტრუქტურა, გაშვების ბრძანებები, ტესტები,
MLflow/DagsHub-ის კონფიგურაცია) იხილეთ [`README.md`](README.md)-ში.*
